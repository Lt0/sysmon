...
let numaMapsHdr = {
    ...
    stack: "由栈使用的内存页数", 
    heap: "由堆使用的内存页数", 
    huge: "Huge 内存页数，显示的是大页内存而不是常规页",
    // total: "总计，包含 stack、heap、huge 以及其它"
}

let File = `内存范围所对应的文件
如果文件被映射为私有
则写访问可能已在此内存范围中生成COW（写时复制）页
这些页面显示为匿名页面`

let Policy = `对该范围内存生效的内存策略
注意，有效策略不一定是该内存范围的进程安装的策略
具体而言，如果进程为该范围安装了 “default” 策略，
则该范围的有效策略将是进程策略，其可能是也可能不是 “default”。`
...
let MapMax = `扫描期间遇到的最大 mapcount（映射单个页面的进程数）
这可以用作在给定存储器范围内发生的共享程度的指示符`

let Active = `活动列表中的页数
仅当与此范围内的页数不同时，才会显示此字段
这意味着存储器范围中存在一些非活动页面
这些页面很快就会被交换器从内存中删除`


let numaMapsDetailsHdr = {
    Addr: "内存范围的起始地址", 
    Policy: Policy, 
    File: File, 
...
exports.numaMapsDetailsHdr = numaMapsDetailsHdr;
exports.other = other;